...
6개의 백엔드 서비스를 운영하면서 모든 서비스마다 Spring Security 설정을 반복하고 있었습니다. 새로운 서비스를 추가할 때마다 인증 로직을 복사-붙여넣기하고, 보안 정책이 변경되면 6개 서비스를 모두 수정해야 하는 상황이었죠. 이는 전형적인 마이크로서비스 아키텍처에서 발생하는 **인증 로직 중복 문제**입니다.
