...
트래픽의 피크가 극단적으로 짧고 높은 경우, 오토스케일링은 **이미 끝난 불에 소방차를 보내는 것**과 같다.
...
안전 마진을 70%로 잡는 이유는, 주문 외에도 DB를 사용하는 API(상품 조회, 쿠폰 조회 등)가 있고, GC나 네트워크 지터 같은 변수를 흡수해야 하기 때문이다. **평균이 아니라 피크를 견딜 수 있는 여유**가 있어야 한다.
...
CircuitBreaker는 가장 극단적인 형태의 Back-pressure다. "느리게 보내는" 수준이 아니라 "아예 안 보내는" 것.
...
> - Circuit Breaker는 가장 극단적인 형태의 Back-pressure다 — 아예 보내지 않는 것
