...
sequenceDiagram
    ...
    Gateway->>Auth Service: 라우팅
    Auth Service->>Auth Service: 사용자 인증
    Auth Service->>Auth Service: JWT 생성
    Auth Service->>Redis: Refresh Token 저장
    Auth Service->>Gateway: 토큰 반환
    Gateway->>Client: 토큰
```
...
sequenceDiagram
    ...
    Client->>Gateway: GET /api/users/me + JWT
    Gateway->>Gateway: JWT 서명 검증
    Gateway->>Redis: 블랙리스트 확인
    Redis->>Gateway: OK
    Gateway->>Gateway: 헤더 추가 (X-User-Id, X-Roles)
    Gateway->>Backend Service: 라우팅
    Backend Service->>Gateway: 응답
    Gateway->>Client: 응답
```
